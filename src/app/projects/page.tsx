                    ...
                    <div key={index} className={'px-6 md:px-12 pt-10'}>
                        <div className='flex flex-col md:flex-row text-wrap gap-10  pb-10'>
                            ...
                            <div>
                                ...
                                <p className={'whitespace-pre-line text-sm md:text-base pt-6 break-normal text-wrap'}>{project.description}</p>
                                <p className={' text-lg md:text-xl opacity-80 font-bold pt-6 italic'}>{'Technologies Used'}</p>
                                <div className={'flex flex-row flex-wrap pt-2'}>
                                    {
                                        project.technologies.map((_, i) => <p key={i}
                                                                              className={'text-s md:text-base pt-1 pr-2 md:pr-5'}>{`${_}${i !== project.technologies.length - 1 ? ',' : ''}`}</p>)
                                    }
                                </div>
